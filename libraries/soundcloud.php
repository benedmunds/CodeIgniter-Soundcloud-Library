...
* Created:  10.20.2009 
* 
* Description:  CodeIgniter SoundCloud API Library based on class written by Anton Lindqvist.  Original license is below.
* 
*/


/**
 * API Wrapper for SoundCloud written in PHP with support for authentication using OAuth.
 *
...
class Soundcloud {
    ...
    function __construct($config) {
		...
		$this->ci->load->config('soundcloud');
		
		if (isset($config) && is_array($config)) {
			$consumer_key       = $config['consumer_key'];
			$consumer_secret    = $config['consumer_secret'];
			$oauth_token        = (isset($config['oauth_token'])) ? $config['oauth_token'] : NULL;
			$oauth_token_secret = (isset($config['oauth_token_secret'])) ? $config['oauth_token_secret'] : NULL;
    	}
    	else {
